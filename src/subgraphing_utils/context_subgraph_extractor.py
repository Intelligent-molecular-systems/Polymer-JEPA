...
def motifContext(graph, sizeContext=0.7):
    ...
    if not intermonomers_bonds: # if no intermonomer bond is found, return all nodes and edges as context
        return torch.ones((1, len(monomer_mask)), dtype=torch.bool), torch.ones((1, graph.num_edges), dtype=torch.bool)
